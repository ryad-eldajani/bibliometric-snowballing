...
class Database
{
    ...
    public function select($sql, $parameters = array(), $connectionType = self::CONNECTION_BS)
    {
         ...
         */
        $connection = $this->connections[$connectionType];
        ...
        // Bind parameters to SQL statement (if any) and retrieve result.
     ...
     * @param array $parameters parameters to bind
     */
    protected function bindParameters(\mysqli_stmt $statement, $parameters = array())
    {
        ...
        // Build $bindParameters, first element is $parameterTypes reference.
        $bindParameters[] = & $parameterTypes;

        // Add the actual parameters to $bindParameters by reference.
